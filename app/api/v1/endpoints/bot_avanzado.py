...
from app.dependencies import get_bot_avanzado
from app.services.rag_avanzado import BotDocumentosAvanzado
from app.clients.paperless import build_document_urls
...
@router.post(
    ...
    summary="Reindexar todos los documentos",
...
async def reindexar(
    ...
    **🔒 Requiere el header `X-Admin-Token`** (definido en `ADMIN_TOKEN`).
    Si `ADMIN_TOKEN` no está configurado, el endpoint queda deshabilitado.
    ...
    try:
        ...
        documentos = [
            DocumentoPaperless(
                ...
                tags=doc.get("tags", []),
                **build_document_urls(doc.get("id"))
            )
